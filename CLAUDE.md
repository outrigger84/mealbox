...
- `/meals` — `GET /` (filters: `?eaten=0`, `?unassigned=1` — the latter now means "not eaten and not linked from any `meal_slots.meal_id`", not the old `assigned_date` check), `POST /` (single manual add — for a meal not from a logged delivery), `POST /batch` (bulk-add with a shared `delivery_date`/`expiry_date`, kept for scripting/back-compat but the UI no longer surfaces it directly), `PATCH /:id`, `POST /:id/eat`, `DELETE /:id`.
...
Calendar rows show the cycle chip plus, for a `subscription` slot, a secondary "Pick meal" control (or the assigned meal's name, tap to reopen the picker; the X on that pill clears it) that opens an inline list of currently-unassigned stock meals. This is deliberately a separate action from the status cycle — see the `meal_slots` note above.
