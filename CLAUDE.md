...
- `/meals` — `GET /` (filters: `?eaten=0`, `?unassigned=1` — means "not eaten and not linked from any `meal_slots.meal_id`", not the old `assigned_date` check — and `?frozen=1`, combinable e.g. `?frozen=1&eaten=0`), `POST /` (single manual add — for a meal not from a logged delivery), `POST /batch` (bulk-add with a shared `delivery_date`/`expiry_date`, kept for scripting/back-compat but the UI no longer surfaces it directly), `PATCH /:id`, `POST /:id/eat`, `POST /:id/freeze` (`{freeze_type}`, sets `frozen_at`/`freeze_type`), `POST /:id/unfreeze` (clears both), `DELETE /:id`.
